...
const initialTypes = [
  { id: "follow-up", label: "Follow Up", icon: "info" as const },
  { id: "phone-call", label: "Phone Call", icon: "delete" as const }
];
...
const TaskType: React.FC = () => {
  ...
  const handleSaveAdd = () => {
    ...
    setTypes((prev) => [
      ...prev,
      { id: `${label.toLowerCase().replace(/\s+/g, "-")}-${Date.now()}`, label, icon: "delete" as const }
    ]);
    handleCancelAdd();
  };
